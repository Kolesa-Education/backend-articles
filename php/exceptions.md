...
ВАЖНОЕ ПРИМЕЧАНИЕ: Здесь я пишу на псевдокоде, на выдуманном Си-подобном языке. Так как в Go нет исключений[^go-panic], а PHP я не знаю хорошо, то раскрываю таким образом механизм общий, для большинства языков. Более детальную работу с исключениями в PHP можно увидеть ниже в статье
...
9. ЕСЛИ `fourth` завершился успешно, то он завершается и его фрейм уходит из стека
